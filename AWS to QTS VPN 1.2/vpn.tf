...
resource "aws_ec2_transit_gateway_route" "vpn_attachment_routes" {
  ...
  transit_gateway_attachment_id = each.key == "vpc_subnet" ? aws_ec2_transit_gateway_vpc_attachment.dev_qts_vpc_tgw_attachment.id : module.vpn_gateway.vpn_connection_transit_gateway_attachment_id
}
